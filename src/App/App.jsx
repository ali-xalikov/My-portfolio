...
function App() {
  ...
  const isCarculation = location.pathname.startsWith("/projects/carculation");
  const isFilms = location.pathname.startsWith("/projects/films");  
  const isTodo = location.pathname.startsWith("/projects/to-do"); 
  const isYemak = location.pathname.startsWith("/projects/yemak"); 
  const isGithub = location.pathname.startsWith("/projects/github"); 
      ...
      >
        <AnimatePresence>
          {isLoading && (
            ...
            >
              <motion.h1
                className="text-5xl md:text-7xl text-teal-400 font-bold"
                initial={{ scale: 0.5 }}
                animate={{ scale: 1 }}
        ...
        <div className="max-w-[100vh] mx-auto">
          <nav className="max-w-[1280px] w-full px-8 py-2 mx-auto flex justify-between items-center mt-5">
            <div
              style={{ color: text }}
              className="font-rubik cursor-pointer text-3xl transition duration-250 hover:text-teal-400"
            >
              Ali Xalikov
            </div>

            <ul
              className="hidden md:flex gap-10 cursor-pointer"
              style={{ color: texts }}
            >
            ...
            <div
              className=" relative max-w-[767px]:block hidden w-8 h-6 cursor-pointer"
            ...
            <div className="flex items-center gap-4 ml-5 max-[767px]:hidden">
              <button
                onClick={() => setMode(!mode)}
                className="bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-md shadow-md transition whitespace-nowrap"
              >
                {mode
          ...
          <hr
            className="max-w-full h-[1px] border-0 mt-2"
          ...
          {burger && (
            <div className="fixed top-0 right-0 w-1/2 h-full bg-[#0f172a] p-5 z-50 border-l border-teal-400 flex flex-col">
              <button
                onClick={() => setBurger(false)}
                className="self-end text-4xl text-teal-400"
              >
                &times;
              </button>

              <ul className="flex flex-col mt-10 gap-8 text-teal-400 text-right cursor-pointer">
                {navItems[language].map((item, idx) => (
                  <li
                    key={idx}
